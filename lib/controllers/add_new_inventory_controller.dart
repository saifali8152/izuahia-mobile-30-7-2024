...
class AddInventoryController extends GetxController {
  GlobalKey<FormState>? inventoryFormKey;
  ...
  TextEditingController? salesPointIdController;
  TextEditingController? boothIdController;
  List<String> inventoryImages = [];

  RxList<SalesPointDatum> salesPointList = RxList();
  RxBool isSalesPointLoading = false.obs;

  RxList<BoothDatum> boothList = RxList();
  RxBool isBoothLoading = false.obs;
  ...
  void onInit() {
    ...
    salesPointIdController = TextEditingController();
    boothIdController = TextEditingController();

    selectSalesPoint().then((value) {
      if (salesPointList.isNotEmpty) {
        selectBooth(spId: salesPointList[0].spId);
      }
    });
  ...
  Future<void> selectBooth({required String spId}) async {
    isBoothLoading.value = true;
    boothList.value = await inventoryRequestRepo.selectBooth(spId: spId);
    boothIdController!.text = salesPointList[0].spId;
    isBoothLoading.value = false;
  }

  Future<void> addNewInventory() async {
    await inventoryRequestRepo.addNewInventory(
      inventoryName: inventoryNameController!.text,
      salesPointId: salesPointIdController!.text,
      boothId: boothIdController!.text,
